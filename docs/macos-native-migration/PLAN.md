...
M11-W4 is complete. The next agent must execute `M11-W5 Launch, stop, tasks, and logs` from `PROGRESS.md`. It must connect launch preparation, Java/Minecraft command construction, `LaunchController`, task observation, cancellation, stop, shutdown, and bounded/redacted logs. Tests must use a fake process executor and disposable instance, verify exact arguments/environment redaction, failure recovery, cancellation, and runtime reconstruction, and must not start Minecraft. Update `PROGRESS.md` and commit detailed evidence before selecting M11-W6.
